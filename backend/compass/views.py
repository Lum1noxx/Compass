...
def a_star_search(start_nodes, end_nodes):
    ...
    # create temporary edges from super start to all start nodes
    super_edges = []
    ...
    while open_set:
        ...
        if current in end_nodes:
            ...
            while current in came_from:
                ...
                edge = Edge.objects.get(start=prev, end=current)
                if edge.type != 'super_edge':
                    path.append(edge)
                current = prev
            path.reverse()
            # delete super start and edges
            super_start.delete()
        ...
        for neighbor in neighbors:
            ...
            if neighbor.adjacent_node not in g_score or tentative_g_score < g_score[neighbor.adjacent_node]:
                came_from[neighbor.adjacent_node] = current
                ...
                if neighbor.adjacent_node not in open_set_hash:
                    heapq.heappush(open_set, (f_score[neighbor.adjacent_node], neighbor.adjacent_node))
